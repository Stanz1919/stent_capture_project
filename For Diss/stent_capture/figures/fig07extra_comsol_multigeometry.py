"""
Fig 7 (extra) — COMSOL multi-geometry gradient comparison (panel a only).
...
linearity check (panel d) are written to the results directory as CSV files:
  ...
  results/fig7extra_powerlawfit_summary.csv  — A, n, R² per geometry
  results/fig7extra_linearity_check.csv      — per-node ratio G[1.5T]/G[0.2433T]
...
def _export_powerlawfit(geom_datasets, labels: list[str]) -> None:
    path = OUT / "fig7extra_powerlawfit_summary.csv"
...
def _export_linearity(d_common, g15, g024, ratio, mean_ratio) -> None:
    path = OUT / "fig7extra_linearity_check.csv"
    expected = 1.5 / 0.2433
    with open(path, "w", newline="") as f:
        w = csv.writer(f)
...
def main():
    print("  Fig 7 (extra): COMSOL multi-geometry gradient profiles...")
    (fig, v1_excel, v2, v3, v5_new,
     d_common, g15, g024, ratio, mean_ratio) = make_figure(y_min=10)

    fig.savefig(OUT / "fig7extra_comsol_multigeometry.png", dpi=150,
                bbox_inches="tight")
    fig.savefig(OUT / "fig7extra_comsol_multigeometry.pdf", bbox_inches="tight")
    plt.close(fig)
    print("  [OK] fig7extra_comsol_multigeometry saved")

    (fig2, *_) = make_figure(y_min=10, x_min=0.025)
    fig2.savefig(OUT / "fig7extra_comsol_multigeometry_ymin10.png", dpi=150,
                 bbox_inches="tight")
    fig2.savefig(OUT / "fig7extra_comsol_multigeometry_ymin10.pdf",
                 bbox_inches="tight")
    plt.close(fig2)
    print("  [OK] fig7extra_comsol_multigeometry_ymin10 saved")
    ...
    fig3.savefig(OUT / "fig7extra_linearity_check.png", dpi=150, bbox_inches="tight")
    fig3.savefig(OUT / "fig7extra_linearity_check.pdf", bbox_inches="tight")
    plt.close(fig3)
    print("  [OK] fig7extra_linearity_check saved")

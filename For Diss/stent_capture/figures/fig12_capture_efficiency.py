"""
Fig 12 — Quantitative capture efficiency curves.
...
from stent_capture.figures.style import COLORS_CODE_DEFAULT, COLORS_THRESHOLD
...
_VELOCITIES = np.append(np.geomspace(0.01, 0.50, 8), 0.60)
_LOADINGS   = np.array([5, 10, 20, 40, 80, 150, 200, 300], dtype=float) * 1e-15
...
_COLOR_V = COLORS_CODE_DEFAULT
_COLOR_L = COLORS_THRESHOLD
...
def make_figure():
    ...
    ax_v.axvline(_V_REF, color='#e67e22', ls='--', lw=1.2, alpha=0.9,
                 label=f'v̄ = {_V_REF:.2f} m/s (Stage 3a ref.)')
    ax_v.axvline(_V_MCA, color='#8e44ad', ls=':', lw=1.2, alpha=0.9,
                 label=f'v̄ = {_V_MCA:.2f} m/s (distal MCA mean, Aaslid 1982)')
    ...
    ax_v.set_title('(a) Efficiency vs mean blood velocity\n'
                   '(200 pg SPION loading, B0 = 1.5 T)', fontsize=10)
    ...
    ax_l.axvline(_M_REF * 1e15, color='#e67e22', ls='--', lw=1.2, alpha=0.9,
                 label=f'{_M_REF*1e15:.0f} pg (Polyak 2008 default)')
    ax_l.axvline(_M_HIGH * 1e15, color='#8e44ad', ls=':', lw=1.2, alpha=0.9,
    ...
    ax_l.set_title('(b) Efficiency vs SPION loading\n'
                   '($\\bar{v}$ = 0.05 m/s, B0 = 1.5 T)', fontsize=10)

    return fig, res_v, res_l


def main():
    print("  Fig 12: Capture efficiency curves…")
    fig, res_v, res_l = make_figure()
    fig.savefig(OUT / "fig12_capture_efficiency.png", dpi=200, bbox_inches="tight")
    fig.savefig(OUT / "fig12_capture_efficiency.pdf", bbox_inches="tight")
    plt.close(fig)
    print("  [OK] fig12_capture_efficiency saved")

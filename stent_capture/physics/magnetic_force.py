...
def _chi_effective(chi_0: float, M_sat: float | None, B_mag: np.ndarray) -> np.ndarray:
    ...
    """
    if M_sat is None:
        # Backward-compatible: constant susceptibility mode
        return np.full_like(B_mag, chi_0)

    # Langevin saturation model: chi_eff(B) = chi_0 * 3*L(xi)/xi
    # where xi = alpha * B / mu_0 and alpha = 3*chi_0 / M_sat
    alpha = 3.0 * chi_0 / M_sat          # units: m/A
    xi = alpha * B_mag / MU_0            # dimensionless

    # Compute Langevin saturation factor f_sat = 3*L(xi)/xi
    # At xi→0: L(xi)/xi → 1/3, so f_sat → 1
    # At large xi: L(xi) → 1, so f_sat → 3/xi → 0
    small = xi < 1e-3
    f_sat = np.empty_like(xi)

    # Taylor expansion for small xi: L(x)/x ≈ 1/3 - x^2/45 + ...
    # So f_sat = 3*L(x)/x ≈ 1 - x^2/15
    f_sat[small] = 1.0 - xi[small]**2 / 15.0

    # For large xi: use coth(xi) = 1/tanh(xi)
    xi_large = xi[~small]

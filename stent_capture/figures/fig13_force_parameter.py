...
# Unified with style.py: gray (baseline), blue (small), orange (moderate),
# red (strong), gray (MRI reference)
# ---------------------------------------------------------------------------
...
def make_figure():
    ...
    # -----------------------------------------------------------------------
    # Panel (c): FP at 200 um vs B0 magnitude (finer sweep)
    # -----------------------------------------------------------------------
    ...
    # --- Panel (c): FP at 200 um vs B0 magnitude ---
    ax_c.plot(B0_range, fp_vs_B0_axial, color=COLORS_CODE_DEFAULT, ls="-", lw=2,
             label="Axial (+z)")
    ax_c.plot(B0_range, fp_vs_B0_trans, color=COLORS_THRESHOLD_HIGH, ls="--", lw=2,
             label="Transverse (+x)")
    ax_c.axhline(fp_axial[0.0][idx_200], color=COLORS_MARKER_REFERENCE, ls=":", lw=1.2,
                 alpha=0.6, label="B0 = 0 baseline")
    ax_c.axvline(1.5, color=COLORS_MARKER_REFERENCE, ls="--", lw=1.2, alpha=0.7,
                label="1.5 T (MRI / COMSOL)")
    ax_c.set_xlabel("B0 magnitude (T)")
    ax_c.set_ylabel("|B_total| · |∇|B_total|| (T²/m)")
    ax_c.set_title("(c) Force parameter at 200 um vs B0 magnitude")
    ax_c.set_xlim(0, 1.5)
    ...
    ax_d.set_xlabel("Distance from stent surface (um)")

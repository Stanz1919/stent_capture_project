...
_COLORS_V = ["#2980b9", "#27ae60", "#e67e22", "#c0392b"]
...
def make_figure():
    ...
    # -----------------------------------------------------------------------
    d_from_wall = np.linspace(0.5e-6, R_ves * 0.99, 300)   # 0.5 µm to ~lumen centre
    ...
    ax_a.set_xlabel("Distance from vessel wall (µm)")
    ax_a.set_ylabel("Stokes drag |F_drag| (pN)")
    ax_a.set_title("(a) Drag on 10 µm cell vs distance from wall")
    ax_a.set_xlim(0, R_ves * 1e6)
    ax_a.legend(fontsize=8)
    # Mark stent inner surface distance from wall
    d_inner_from_wall = R_ves - r_inner
    ax_a.axvline(d_inner_from_wall * 1e6, color="#888888", ls=":",
                 lw=1.2, label=f"Stent inner surface ({d_inner_from_wall*1e6:.0f} µm)")
    ...
    for v_mean, col in zip(_V_CASES, _COLORS_V):
        ax_b.plot(r_profile * 1e3, v_profiles[v_mean],
                  color=col, lw=2, label=f"v_mean = {v_mean:.2f} m/s")
    ax_b.axvline(r_inner * 1e3, color="#888888", ls=":", lw=1.2)
    ax_b.text(r_inner * 1e3 + 0.02, ax_b.get_ylim()[1] * 0.5 if ax_b.get_ylim()[1] > 0 else 10,
              "Stent\ninner\nsurface", fontsize=7, color="#888888")
    ...
    # --- Panel (c) ---
    ax_c.semilogy(v_range * 100, drag_vs_v, "k-", lw=2.5, label="Stokes drag at stent inner surface")
    ref_colors = ["#555555", "#e67e22", "#c0392b"]
    ...
    # --- Panel (d) ---
    ax_d.plot(v_range * 100, wss_vs_v, "b-", lw=2)
    ax_d.axhspan(1.5, 7.0, alpha=0.08, color="green", label="Atheroprotective (1.5–7 Pa)")
    ax_d.axhspan(0.0, 0.4, alpha=0.1, color="red",   label="Atherogenic (< 0.4 Pa)")

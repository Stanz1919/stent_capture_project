...
(c) |F_mag| vs circumferential angle at fixed r = r_outer + 200 µm,
    B0 = 0.5 T axial. Polar plot showing 8-strut periodicity.
    B0 = 0 reference overlaid as dashed.
(d) |F_mag| at 200 µm vs SPION load (1–100 pg), three B0 values.

Default cell: 10 µm radius, 10 pg iron oxide, chi = 2.0.
Default stent: R=1.5mm, 8 struts, 100x80 µm, M=1 MA/m, assume_saturation=True.
...
def make_figure():
    ...
    # Panel (c): polar plot — |F_mag| vs angle at r_outer + 200 µm
    R = DEFAULTS["R"]
    t = DEFAULTS["t"]
    r_obs = R + t / 2 + 200e-6
    n_phi = 360
    ...
    ax_d.set_title("(d) Force vs SPION load (at 200 µm, axial B0)")

...
- Stent: 8 struts, R = 1.5 mm, 80 um thick, M = 2.20 MA/m (COMSOL-calibrated at B0=1.5T)
...
def _make_panel_b():
    ...
    ax.set_xlabel("Distance from stent outer surface (um)", fontsize=10, fontweight="bold")
...
def _make_panel_c():
    ...
    ax.set_ylabel("Static capture distance (um)", fontsize=10, fontweight="bold")
    ax.set_xlim(7, 250)
...
def make_figure():
    ...
    axes[1].semilogy(d_range * 1e6, F_mag_constant, "--s", color=_COLOR_CONSTANT, lw=2.0, ms=4,
                    label="Constant (linear model)", zorder=4)
    axes[1].set_xlabel("Distance from stent outer surface (um)", fontsize=10, fontweight="bold")
    ...
    axes[2].set_ylabel("Static capture distance (um)", fontsize=10, fontweight="bold")
    ...
    fig.suptitle(
        ...
        "At MRI strength (1.5T), Langevin model gives 5.7x lower susceptibility; force scales accordingly.",

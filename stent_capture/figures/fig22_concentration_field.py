...
def generate(show: bool = False) -> None:
    ...
    for ax, C, title, mult in [
        (ax1, C_b, "Basal secretion (1x)", 1),
        (ax2, C_e, "VEGF-enhanced (100x)", 100),
    ]:
        vmax = max(np.percentile(C, 99.5), C_THERAPEUTIC_LOW * 2)
        im = ax.pcolormesh(z_um, x_um, C, shading="auto", cmap=COLORS_CONCENTRATION,
                           vmin=0, vmax=vmax)
        fig.colorbar(im, ax=ax, pad=0.02, label="VEGF concentration (ng/mL)")
        ...
        ax.text(0.02, 0.98, f"{n_total} cells", transform=ax.transAxes, fontsize=8,
                ...
                bbox=dict(boxstyle="round,pad=0.3", fc="black", alpha=0.6, edgecolor="white", linewidth=0.5))

    ax1.set_ylabel("Circumferential position (um)")

    fig.suptitle("Figure 22: Steady-state VEGF Distribution on Vessel-Wall Surface",

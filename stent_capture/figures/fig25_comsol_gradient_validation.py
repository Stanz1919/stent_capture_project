...
def _code_gradient_profile(
    ...
    """Analytical ``|grad_B`` along the strut-aligned axis vs distance from surface."""
...
def _compute_crossings(d_m: np.ndarray, grad: np.ndarray) -> dict[str, float]:
    """Linearly interpolate the distances at which *grad* crosses each threshold (returns um)."""
    out: dict[str, float] = {}

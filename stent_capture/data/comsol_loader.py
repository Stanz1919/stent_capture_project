...
class ComsolDataset:
    ...
    grad_T_per_m: np.ndarray                 # |∇|B|| (T/m)
    fit_A: float | None = None               # prefactor in G = A d^n
    fit_n: float | None = None               # exponent
...
def _first_num_col_indices(header_row: tuple) -> dict[str, int]:
    ...
    for i, cell in enumerate(header_row):
        ...
        if h.startswith("d from strut"):
            mapping["d"] = i
        elif h.startswith("|") and "b" in h:  # |∇B| column header
            mapping["grad"] = i
    return mapping
...
def _parse_sheet(wb, key: str) -> ComsolDataset:
    ...
    # Locate the header row: it's the row containing "d from strut (mm)".
    ...
    for row in ws.iter_rows(min_row=header_idx + 2, values_only=True):
        ...
        try:
            ...
            continue  # skip non-numeric rows (trendline blocks, text rows)

    d_mm = np.asarray(d_vals, dtype=float)
    grad = np.asarray(g_vals, dtype=float)

    # Sort by distance so downstream consumers get a monotone axis.
...
def _extract_fit_from_summary(wb) -> dict[str, dict]:
    """Parse the per-sheet trendline summary from the top-level Summary sheet."""
    ws = wb["Summary"]
    fits: dict[str, dict] = {}

    # Find the row whose first cell is exactly "Dataset".
    header_idx: int | None = None
    for i, row in enumerate(ws.iter_rows(values_only=True)):
